        ...
        private DICOMSliceFile ReadDICOMFile(string filePath)
        {
            ...
            if (file != null && file.HasPixelData)
            {
                ...
                else if (file.DataSet.Contains(posTag))
                {
                    ...
                    slice.position.x = (float)Convert.ToDouble(elemLoc.Value[0]);
                    slice.position.y = (float)Convert.ToDouble(elemLoc.Value[1]);
                    slice.position.z = (float)Convert.ToDouble(elemLoc.Value[2]);
                    slice.position = pos;
